...
#generatore dei registri virtuali 
def new_virtual_reg():
...
class AssignStat(Stat):
    ...
    def lower(self):
        ...
        def lower_auxiliary(node):
            ...
            for child in node.children:
                ...
                if get_type(child)=="Var" or get_type(child)=="Const":
                    ...
                    if get_type(child)=="Var":
                     s = child.symbol
                    else:
                     s = child.value
                    load_stat = LoadStat(symbol=s , register=new_reg , symtab=self.symtab)
        ...
        if get_type(self.expr)=="BinExpr" or get_type(self.expr)=="UnExpr":
         ...
         reg = new_virtual_reg()
         if get_type(self.expr)=="Var":
             s = self.expr.symbol
         else:
             s = self.expr.value
         load_stat = LoadStat(symbol= s , register=reg , symtab=self.symtab)
         store_stat = StoreStat( what= reg , symbol=s, symtab=self.symtab)
         stat_list.append(load_stat)
         stat_list.append(store_stat)
         return self.parent.replace(self,stat_list)
